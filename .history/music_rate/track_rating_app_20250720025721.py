...
def open_in_obsidian(filepath):
    ...
    if os.name == 'nt':  # Windows
        # Попытка открыть через URI схему Obsidian
        try:
            # Преобразуем путь в URI формат для Obsidian
            file_uri = f"obsidian://open?vault=rating_result&file={os.path.basename(filepath)}"
            subprocess.Popen(['start', file_uri], shell=True)
        except:
            # Если не получилось, пробуем открыть через командную строку Obsidian
            try:
                # Путь к Obsidian (может отличаться)
                obsidian_paths = [
