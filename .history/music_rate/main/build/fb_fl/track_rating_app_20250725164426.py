...
root = tb.Window(themename="superhero")  # или другой стиль
...
def update_score():
    ...
    vibe_val = sliders[4].get()
    vibe_multiplier = 1.0 + (vibe_val - 1) * (1.25 / 9)
    vibe_percent = int(round((vibe_multiplier - 1.0) * 100))
    total_score = round(main_score * vibe_multiplier)

    # Обновляем отображение значения справа от ползунка
    if hasattr(sliders[4], 'value_var'):
        sliders[4].value_var.set(str(vibe_val))

    score_var.set(
        f"{sliders[0].get()} + {sliders[1].get()} + {sliders[2].get()} + {sliders[3].get()} + {vibe_val} ( +{vibe_percent}% ) = {total_score} БАЛЛОВ"
    )
...
def calculate_score():
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"- **{labels[4]}**: {vibe_val} ( +{vibe_percent}% )\n")
...
for i, (text, max_val) in enumerate(criteria):
    ...
    if i == 4:
        ...
        value_label = tk.Label(row, textvariable=value_var, width=3)
        value_label.pack(side="right")
        val.value_var = value_var
        val.value_label = value_label
    else:
