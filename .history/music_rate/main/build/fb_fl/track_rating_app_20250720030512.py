...
def calculate_score():
    ...
    # Если нашли Obsidian vault, создаем папку там, иначе в main
    if obsidian_dir:
    ...
    score_var.set(f"Файл создан: {md_path}")

def open_in_obsidian(filepath):
    if os.name == 'nt':
        # Пытаемся открыть файл в Obsidian разными способами
        try:
            # Способ 1: Через obsidian:// URI с относительным путем
            abs_path = os.path.abspath(filepath)
            # Получаем относительный путь от корня диска
            relative_path = abs_path.replace(':', '').replace('\\', '/')
            uri = f'obsidian://open?file=/{relative_path}'
            subprocess.run(['start', uri], shell=True, check=True)
        except:
            try:
                # Способ 2: Через obsidian:// URI с абсолютным путем
                abs_path = os.path.abspath(filepath)
                uri = f'obsidian://open?file={abs_path}'
                subprocess.run(['start', uri], shell=True, check=True)
            except:
                try:
                    # Способ 3: Прямое открытие в Obsidian через командную строку
                    subprocess.run(['obsidian', filepath], check=True)
                except:
                    # Способ 4: Обычное открытие файла
                    os.startfile(filepath)
    elif os.name == 'posix':
        try:
            # Для macOS и Linux
            subprocess.run(['open', '-a', 'Obsidian', filepath], check=True)
...
def open_folder():
    ...
    # Если нашли Obsidian vault, используем папку там, иначе в main
    if obsidian_dir:
        rating_dir = os.path.join(obsidian_dir, "rating_result")
    else:
        rating_dir = os.path.join(main_dir, "rating_result")
