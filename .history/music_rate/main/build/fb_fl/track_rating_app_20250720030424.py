...
def open_in_obsidian(filepath):
    if os.name == 'nt':
        # Пытаемся открыть файл в Obsidian через командную строку
        try:
            # Попытка открыть через obsidian:// URI
            subprocess.run(['start', f'obsidian://open?file={filepath}'], shell=True, check=True)
        except subprocess.CalledProcessError:
            # Если не получилось, пробуем через обычное открытие файла
