...
def open_in_obsidian(filepath):
    if os.name == 'nt':
        # Пытаемся открыть файл в Obsidian
        try:
            # Способ 1: Прямое открытие в Obsidian через командную строку
            subprocess.run(['obsidian', filepath], check=True)
        except:
            try:
                # Способ 2: Через obsidian:// URI (более простой формат)
                abs_path = os.path.abspath(filepath)
                # Кодируем путь для URI
                import urllib.parse
                encoded_path = urllib.parse.quote(abs_path)
                uri = f'obsidian://open?file={encoded_path}'
                subprocess.run(['start', uri], shell=True, check=True)
            except:
                try:
                    # Способ 3: Пытаемся найти Obsidian в стандартных местах
                    obsidian_paths = [
                        os.path.expanduser("~/AppData/Local/Programs/Obsidian/Obsidian.exe"),
                        "C:/Program Files/Obsidian/Obsidian.exe",
                        "C:/Program Files (x86)/Obsidian/Obsidian.exe"
                    ]
                    for obsidian_exe in obsidian_paths:
                        if os.path.exists(obsidian_exe):
                            subprocess.run([obsidian_exe, filepath], check=True)
                            break
                    else:
                        # Способ 4: Обычное открытие файла
                        os.startfile(filepath)
                except:
                    # Способ 5: Обычное открытие файла
                    os.startfile(filepath)
    elif os.name == 'posix':
        try:
            # Для macOS и Linux
