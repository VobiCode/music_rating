...
def open_in_obsidian(filepath):
    if os.name == 'nt':
        # Создаем временный .bat файл для открытия Obsidian
        try:
            # Создаем .bat файл в той же папке, где создан markdown файл
            bat_dir = os.path.dirname(filepath)
            bat_path = os.path.join(bat_dir, "open_in_obsidian.bat")
            
            # Содержимое .bat файла
            bat_content = f'''@echo off
REM Попытка открыть файл в Obsidian
start "" "{filepath}"
REM Если Obsidian не открыл файл, открываем папку
explorer "{bat_dir}"
'''
            ...
            threading.Thread(target=delete_bat, daemon=True).start()
            
        except Exception as e:
            # Если что-то пошло не так, просто открываем файл обычным способом
